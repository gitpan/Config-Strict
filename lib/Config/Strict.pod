...
Config::Strict - Add strict type-checking to configuration data
...
parameter type-checking to configuration data. Both built-in and custom types
can be used to build a validation profile for the entire configuration.
...
Any parameter not named here cannot be added later - any attempt to access a
parameter not provided in the constructor will result in an error.
...
The special value '_all' will require all parameters to be required.
...
These methods will die with a stack trace if a parameter doesn't exist in the
configuration profile, or if an attempt is made to set a parameter with an
invalid value.
...
=head2 $obj->get_profile( $param )

Returns the coderef "profile" used to validate C<$param>. It takes a single 
argument and returns true if the passed argument would be a valid value for 
C<$param>. If it's a L<Declare::Constraints::Simple> profile (true for all cases
except custom bare coderefs) it will more specifically return a 
